import operator
import random

class Species:
    def __init__(self, player):
        self.players = []
        self.average_fitness = 0
        self.threshold = 1.2
        self.players.append(player)
        self.benchmark_fitness = player.fitness
        self.benchmark_perceptron = player.perceptron.clone()
        self.champion = player.clone()
        self.staleness=0

    def similarity(self, perceptron):
        similarity = self.weight_difference(self.benchmark_perceptron, perceptron)
        return self.threshold > similarity

    @staticmethod
    def weight_difference(per1, per2):
        total_weight_difference = 0
        for i in range(len(per1.connections)):
            for j in range(len(per2.connections)):
                if i == j:
                    total_weight_difference += abs(per1.connections[i].weight - per2.connections[j].weight)
                    
        return total_weight_difference
    
    def add_to_species(self, player):
        self.players.append(player)

    def sort_players_by_fitness(self):
        self.players.sort(key=operator.attrgetter('fitness'), reverse = True)
        if len(self.players) and self.players[0].fitness > self.benchmark_fitness:
            self.benchmark_fitness = self.players[0].fitness
            self.champion = self.players[0].clone()
        else:
            self.staleness +=1

    def calculate_average_fitness(self):
        total_fit = 0
        for p in self.players:
            total_fit += p.fitness
        if self.players:
            self.average_fitness = int(total_fit/len(self.players))
        else:
            self.average_fitness = 0
    
    def offspring(self):
        if self.players:
            baby = self.players[random.randint(1, len(self.players))-1].clone()
        else:
            baby = self.champion.clone()
        baby.perceptron.mutate()
        return baby